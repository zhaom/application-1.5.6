...
MAIN_CLASS="com.babeeta.butterfly.application.router.gateway.GatewayRouterServer"
...
if test -z $1 ; then
	echo 'Usage service.sh [start|debug|stop] listen_address secondary_router_list'
	exit 1
else
	if ! test -x $JAVA ; then
	...
	case $1 in
		'start' )
			if ! test -z `cat $PID` ; then
				...
				echo $! > $PID
				log_success_msg "Starting Router Server on $2....."
				exit 0;
			fi
		;;
		'debug' )
			...
			if test -z `cat $PID` ; then
				...
				exec $JAVA_HOME/bin/java $JAVA_OPTS  -classpath $CLASSPATH $MAIN_CLASS $APP_OPTS>> $LOG/std.out 2>&1 &
				echo $! > $PID
				log_success_msg 'Starting Tunnel Server in debug mode......'
				exit 0;
			fi
		;;
		'stop' )
			if ! test -z `cat $PID` ; then
				...
				log_failure_msg "Router Server on $2 is not running. No need to stop it"
				exit 1;
			fi
		;;
	esac
